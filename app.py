...
import cif_editor as cif_edit # ✅ 新規インポート
...
# --- UI定義 ---
# ✅ 修正点: タブにCIF Structure Editorを追加
...
with tab1:
    ...
    with col1:
        st.subheader("▶️ Now Running"); st.info(st.session_state.current_job or "No job is currently running.")
    with col2:
        st.subheader("📋 Waiting Queue")
        if st.session_state.job_queue:
            st.dataframe(pd.DataFrame([item['project_name'] for item in st.session_state.job_queue], columns=["Queued Project Name"]))
        else:
...
with tab3:
    ...
    if editor_file:
        ...
        with col1:
            replace_from = st.text_input("Element to Replace", "Bi")
        with col2:
            replace_to = st.text_input("Replace With", "Mg")
        with col3:
            percentage = st.number_input("Percentage to Replace (%)", min_value=0.0, max_value=100.0, value=10.0, step=1.0)
        
        with col4:
            st.write("") # スペーサー
            st.write("") # スペーサー
            generate_button = st.button("Generate Modified CIF", type="primary")
        ...
        if generate_button:
            ...
            if "Success" in " ".join(log_messages):
                ...
                st.info("You can now drag this downloaded file to the uploader in the sidebar to run a simulation.")
...
with st.sidebar:
    ...
    if not allow_overwrite:
        project_prefix = st.text_input("Project Name Prefix", value="run2")
    ...
    with st.expander("NPT Simulation", expanded=True):
        ...
        temp_start = st.number_input("Start Temp (K)", 1); temp_end = st.number_input("End Temp (K)", 800)
        temp_step = st.number_input("Temp Step (K)", 1); eq_steps = st.number_input("Steps per Temp", 100)
    ...
    if st.button("➕ Add to Queue", type="primary", use_container_width=True):
        for uploaded_file in uploaded_files:
            ...
            if not allow_overwrite and os.path.exists(os.path.join(PROJECTS_DIR, project_name_candidate)):
                if not project_prefix: st.sidebar.error("Prefix is required."); continue
...
if st.session_state.is_running:
    ...
    job_parts = project_name.split('_'); original_filename = ""
    ...
    try:
        ...
        def update_progress(current, total, message, partial_df):
            ...
            if not partial_df.empty:
                fig_abc, ax_abc = plt.subplots(); ax_abc.plot(partial_df.index, partial_df[['a_lengths', 'b_lengths', 'c_lengths']]); ax_abc.set_xlabel("Steps"); ax_abc.set_ylabel("Å"); ax_abc.grid(True); ax_abc.legend(['a','b','c']); chart_placeholder_abc.pyplot(fig_abc, clear_figure=True); plt.close(fig_abc)
                fig_v, ax_v = plt.subplots(); ax_v.plot(partial_df.index, partial_df['volumes'], color='g'); ax_v.set_xlabel("Steps"); ax_v.set_ylabel("Å³"); ax_v.grid(True); chart_placeholder_v.pyplot(fig_v, clear_figure=True); plt.close(fig_v)
        traj_filepath = os.path.join(project_path, "trajectory.xyz")
        npt_df = sim.run_npt_simulation_parallel(initial_atoms=opt_atoms, model_name=selected_model, sim_mode=sim_mode, magmom_specie=magmom_specie, temp_range=(temp_start, temp_end, temp_step), time_step=1.0, eq_steps=eq_steps, pressure=1.0, n_gpu_jobs=n_gpu_jobs, progress_callback=update_progress, traj_filepath=traj_filepath)
        if not npt_df.empty:
            elapsed_time = time.time() - start_time
            with open(os.path.join(project_path, "execution_time.txt"), "w") as f: f.write(f"{elapsed_time:.2f}")
            fig_temp = viz.plot_temperature_dependent_properties(npt_df, 100); fig_temp.savefig(os.path.join(project_path, "npt_vs_temp.png"))
